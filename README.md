...
The first rare-variant set-test foundation is available as a deterministic FP64 CPU reference API, not yet as a command-line analysis mode. It defines explicit variant-to-set membership, REF/ALT and effect-allele validation, positive weights, aligned-cohort MAF/MAC masks, overlapping sets, mean/zero/error genotype missingness, reusable continuous-trait null models, and audited error-or-skip behavior for absent, empty, and monomorphic sets. See [docs/SET_TEST_FOUNDATION.md](docs/SET_TEST_FOUNDATION.md) for the frozen input and scientific contracts. Production CLI integration, bounded streaming/checkpointing, and independent region-to-variant expansion remain before burden analysis is user-facing.
...
Genotype-outer remains more expensive than complete-trait analysis because every genotype block must be evaluated against every exact sample/covariate mask. The on-device compact finalizer removes the former large `X'g` downloads. On the batch1234 2,048-variant CSV fixture with 16,862 masks, 512-row genotype blocks completed 358,621,184 comparisons in 230.43 seconds of analysis wall time and retained the same identifiers/N/DF as 256-, 1,024-, and 2,048-row runs. This is a workload-specific RTX 2080 measurement, not a universal block default; a full incomplete-trait chromosome remains a long operational run even though it is no longer blocked on the former host-transfer design.
...
QC is parallelized across variants after sample alignment. `--variant-qc-threads 0` (the default) automatically selects a bounded worker count, normally leaving one logical processor free and capping the pool at 8; multi-core machines use at least two variant workers. Set a positive value to override it, or `1` for the sequential HTSJDK reference path. The cap reflects a 4,746-sample benchmark where throughput plateaued at eight workers; more threads increased contention without improving throughput. The INI equivalent is `variant_qc_threads`. VCF traversal and compressed input remain sequential, but workers directly scan only the raw FORMAT, GT, DS, and FT fields needed for aligned-sample dosage counts, MAF/MAC classification, and exact HWE; they do not allocate a full HTSJDK `Genotype` object and attribute structures for every sample. BCF genotype expansion remains on the reader thread because its lazy decoder shares mutable builders. The bounded queue retains up to four in-flight records per worker and the completion summary reports measured average active worker tasks and the count of low-allocation VCF QC parses. Completed records are still consumed in source order, so the QC report, duplicate detection, retained-variant order, and later association identifiers are deterministic. Matrix-header samples excluded by alignment do not contribute to dosage/QC calculations. The direct path is continuously compared byte-for-byte with the one-worker HTSJDK path for sorted/unsorted sample headers, sample subsets, filtered and partial calls, changing FORMAT layouts, and missing values. The first scan retains only a compact inclusion/EAF decision per input record; later source passes reuse those decisions rather than recomputing frequency and HWE.

The aligned-sample QC scan is resumable. Ordered batches are committed atomically under `<variant-qc-output>.checkpoint/<signature>/` by default; `--variant-qc-checkpoint DIR` (INI: `variant_qc_checkpoint`) changes the root. The signature includes the normalized genotype path, source and index size/modification time, resolved VCF/BCF field and filtering policies, normalized regions, complete header sample list, and final aligned sample indices/order. A policy, file/index metadata, region, or alignment change therefore starts a separate state directory instead of silently reusing obsolete EAF/MAF/MAC/HWE decisions. Do not edit checkpoint parts. A process lock rejects simultaneous writers to the same signature. Indexed region scans and sorted-header VCF/BCF inputs seek directly to the last durable canonical variant and verify that boundary. For an unrestricted VCF whose header samples are unsorted, HTSJDK's indexed reader would eagerly decode genotypes serially; resume instead validates the completed compressed prefix without genotype expansion and then continues through the direct parallel QC path. This prefix pass reports progress and does not recompute completed frequency/HWE results. A fully completed matching checkpoint is loaded directly and can recreate a missing QC TSV without opening a full variant-record pass. Checkpoints are retained intentionally for later reruns; remove a signature directory only when its analysis will not be resumed.
...
For 114,406 traits by 4,746 samples, the numeric FP64 values alone require about 4.05 GiB, before Java row/identifier overhead and active worker buffers. Therefore “in memory” means once-prepared, chunk-built host residency—not one monolithic GPU projection or one giant GPU allocation. Genotype-outer exact deletion residualizes each trait once within its own mask, pads excluded samples with zero, and stores all estimable traits in one reusable prepared cache. Pattern-outer compatibility mode still prepares one cache per mask.
...
The default directory is `<output-file>.checkpoint`. `--checkpoint-dir DIR` selects a different location, and `--keep-checkpoints` retains completed parts after successful assembly. Pattern-outer exact deletion commits one ordered result group per mask and nests genotype-block state within each group. Genotype-outer exact deletion instead commits ordinary genotype-block parts spanning all estimable traits; an interrupted identical `--resume` run reuses the signed missingness scan, global prepared trait cache, and completed result blocks. Resume is rejected when input/cache signatures, sample masks/order, covariate values, predictor missing policy, precision, thresholds, degrees of freedom, block sizes, or output modes differ. The corresponding INI keys are `cache_dir`, `rebuild_cache`, `checkpoint_dir`, `resume`, `keep_checkpoints`, `max_trait_patterns`, `trait_pattern_scheduler`, and `unestimable_trait_patterns`.
