...
Genotype-outer is currently a correctness-first implementation. On the batch1234 incomplete-trait production fixture, a 146-variant slice was exact and restartable but reached about 0.94 million comparisons/s, substantially below the complete-trait scheduler. Do not extrapolate that slice into an expensive full-chromosome run until the on-device pattern-statistics reduction listed in `TODO.md` is complete. The reusable signed scans, trait cache, pattern audit, and completed block remain valid development/validation artifacts.
