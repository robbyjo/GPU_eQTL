...
Exact deletion now performs a preflight before building pattern-specific caches. It reports the number of patterns, observed-sample range, approximate full-cohort predictor-preparation passes, and an upper bound for numeric prepared-predictor storage. Patterns with too few complete samples for the selected covariate design, tested predictor, and `df_offset` are rejected before association work. To prevent accidental multi-terabyte schedules, `--max-trait-patterns` defaults to 256; set a larger value only after reviewing the report, or `0` to disable the count limit. Disabling the limit does not make the work smaller. For high-entropy missingness, inspect first and consider trait mean/zero imputation or prefiltering until the planned genotype-block-outer implementation removes the current repeated-predictor preparation.
...
The default directory is `<output-file>.checkpoint`. `--checkpoint-dir DIR` selects a different location, and `--keep-checkpoints` retains completed parts after successful assembly. In exact trait-pattern mode, one durable ordered result group is committed per pattern and each in-progress group has its own genotype-block checkpoint; final assembly occurs only after every group is complete. Resume skips complete groups and continues an interrupted group. Resume is rejected when input/cache signatures, sample masks/order, covariate values, precision, thresholds, degrees of freedom, block sizes, missing/frequency policies, or output modes do not match. The corresponding INI keys are `cache_dir`, `rebuild_cache`, `checkpoint_dir`, `resume`, `keep_checkpoints`, and `max_trait_patterns`.
