...
BGZF VCF plus tabix (`.tbi`) and BCF/VCF plus an HTSJDK Tribble index (`.idx`) support true interval queries. A neighboring index is detected automatically; use `--variant-index FILE` to specify it. Production comparison against bcftools 1.24 covered overlapping interval unions, exact membership/order, aligned-sample AC/AN/NS, MAF/HWE, and interrupted BCF restart; tabix VCF and Tribble-indexed BCF produced byte-identical QC. Standard bcftools CSI (`.csi`) is recognized as an index candidate but is not yet decoded by the current pure-Java HTSJDK reader; region queries with an unsupported CSI fail before analysis rather than silently scanning the whole file. For unrestricted input, an otherwise unsupported neighboring CSI is ignored in favor of the valid sequential source. Plain gzip VCF remains valid for unrestricted sequential analysis but cannot service a region request.
