...
## 2026-08-27T20:07:56-04:00 — Heap tuning, backend validation, pattern QC, and production BCF audit

### Baseline and goal

- Baseline commit: `81b7ab65799ee64a899987c450e604b203984d38` (`Add scalable sliding-window set tests`) on `master`; both `origin/master` and `github/master` matched and the worktree was clean at the start.
- Goal: complete the five requested follow-ups: add heap-aware automatic set tiles; validate forced OpenBLAS/Java complete analyses against CUDA FP64; benchmark oneMKL against OpenBLAS; make genotype-outer per-pattern aggregate variant QC durable; and validate production BCF/indexed regions against bcftools before deciding whether CSI should be added.

### Decisions and files changed

- Changed omitted/zero `--set-block-size` from a fixed 256 to automatic. `QSetTestRunner` estimates a worst resident tile from the aligned sample count, selected unique dosage rows, actual membership density, burden/SKAT/SKAT-O workspaces, a 25% safety factor, and a 256-MiB reserve. It uses at most half of the JVM maximum heap, caps the recommendation at 256, prints the estimate/budget, and puts the resolved value in the checkpoint signature. Positive values remain explicit overrides and negative values fail validation.
- Extended compact genotype-outer pattern statistics from three to five FP64 values per pattern/variant: replacement, residual sum of squares, filled sum of squares, called count, and dosage sum. Portable CPU, CUDA/NVRTC, and OpenCL finalizers use the same layout.
- Added `QGenotypeOuterPatternQcCheckpoint`: every successfully completed association genotype block now has a matching signature-bound, checksummed, atomically written QC part. Resume reuses a result block only when both parts validate. Deterministic assembly writes `<output>.pattern-variant-qc.tsv` with input, included, no-call, pattern-monomorphic, missing-genotype, and candidate pattern-MAF/MAC counts for every pattern. Candidate frequency counts are audited but not applied, so scalable genotype-outer still rejects `--frequency-scope pattern` until filtering semantics are enabled deliberately.
- Made index selection prefer `.tbi`, then HTSJDK Tribble `.idx`, then `.csi`. Standard CSI is recognized but remains unsupported by HTSJDK 5: a region request now fails before QC with an explicit tabix/Tribble requirement and a no-sequential-fallback statement; unrestricted input ignores an otherwise unsupported neighboring CSI with a warning and uses valid sequential traversal.
- Changed `QeQTLAnalysisConfig.java`, `QSetTestRunner.java`, `QGenotypeOuterPatternJob.java`, `QeQTLAnalysis.java`, `QVariantMatrixSource.java`, `GpuPatternStatisticsResult.java`, `GpuPatternStatisticsSupport.java`, `CudaGpuContext.java`, and `JoclGpuContext.java`; added `QGenotypeOuterPatternQcCheckpoint.java`; expanded CLI, set-runner, pattern-analysis, variant-source, and GPU integration tests; updated `README.md`, `TODO.md`, `docs/COMMAND_LINE_REFERENCE.md`, and `docs/SET_TEST_FOUNDATION.md`.

### Verification and production evidence

- Focused command: `.\mvnw.cmd -q '-Dtest=QVariantMatrixSourceTest,QTraitPatternAnalysisTest,QSetTestRunnerTest,QeQTLCommandLineTest,GpuKernelIntegrationTest' test` — passed after the final CSI behavior change. Earlier focused runs also passed the new automatic heap and compact-pattern-QC assertions. One intermediate set-runner test failed because its explicit one-set output-order fixture had accidentally been changed to automatic sizing; restoring its intentional explicit block size fixed the fixture without changing production behavior.
- Full command: `.\mvnw.cmd test` — the first sandboxed attempt could not reach Maven Central for the declared antrun plugin; the authorized rerun passed 124 tests with zero failures/errors and five intentional hardware/platform skips in 1:04. Available CUDA integration tests passed. `.\mvnw.cmd -q -DskipTests package` then built the final universal jar (135,895,444 bytes, SHA-256 `64F3371DB0B67F352059AB3081A50F57488688B448C0026AC63258DB08DDE9E5`). Jar inspection found `META-INF/LICENSE`, `META-INF/LICENSE_EXCEPTION`, and all three files under `META-INF/licenses/`.
- Heap-tuning production command used `java -Xmx8g -Deqtl.cpu.blas=openblas --enable-native-access=ALL-UNNAMED -jar target\gpu-eqtl-2.0.0-SNAPSHOT-all.jar` with the batch1234 `chr22-filtered-batch1234-dose-meanimp.csv`, all 4,746 covariate samples, two traits, `Age`, burden, `--window-size 100000 --window-stride 50000`, `--expression-block-rows 1`, and omitted `--set-block-size`. Automatic sizing selected 70 sets from a 3,956,932,400-byte worst-tile estimate under a 4,026,531,840-byte budget. All 765 windows completed in 11 blocks and 172.15 seconds; the observed JVM heap peak at tile boundaries was 5,406,173,208 bytes versus 7,905,198,712 with explicit 256. The result-row multiset matched the prior explicit-size output exactly, and the 26,959,285-byte audit was byte-identical (SHA-256 `0BEBD9D1463113483B845D8887BACF6A1728AADE11DBA4338BDA6CF10F62EFD6`). Different legal tile sizes retain tile-major output order, so the complete result file hashes differ even though every line is present unchanged.
- Complete-backend validation derived 513 predictor rows and 129 complete trait rows from batch1234, retained all 4,746 samples, adjusted for `Age,Sex,PCE1`, forced CPU residualization and disk trait cache, requested predictor/trait blocks 257/64 (effective predictor blocks 272+241 and trait blocks 64+64+1), and used `--threshold p 0.05`. Forced `-Deqtl.cpu.blas=openblas`, `-Deqtl.cpu.blas=java`, and CUDA FP64 each completed 66,177 comparisons and retained the same 5,115 ordered identifiers with exact N/DF. Maximum absolute OpenBLAS differences from CUDA were `2.43e-17` R-squared, `9.71e-17` effect, `7.99e-15` T, and `3.55e-14` log10-p; Java maxima were `2.36e-16`, `9.16e-16`, `4.44e-14`, and `2.70e-13`. GPU-compute totals were 0.168 seconds CUDA, 0.429 OpenBLAS, and 0.640 Java. Each backend produced exactly two genotype-block checkpoint parts; a `--resume` rerun preserved its output byte-for-byte with SHA-256 CUDA `4276272381E8F6F307DD5E4A6C925F5FADA0B9CA784BBB1CF989441AC156F060`, OpenBLAS `9C1A595FD8ECAD9402FDB528FFD5899E6C29A774D0CBBC66C5BEDE26000D9CCD`, and Java `B906CC4F1699CE9EBF6907B60A56F0B98E862787827495D55C1BF963D9B7C57E`.
- oneMKL command: `.\mvnw.cmd -q -Pcpu-mkl-windows-x86_64 -DskipTests package`; the isolated 249,269,964-byte jar was run with `-Deqtl.cpu.blas=mkl` and `openblas` over a production-derived 8,193-predictor by 1,025-trait, 4,746-sample schedule, `Age,Sex,PCE1`, 1,024/256 tiles (partial final one-row tiles), 45 compute calls, 8,397,825 comparisons, and p=0.001. After one warmup per engine, two timed OpenBLAS runs had analysis/compute seconds 5.123/3.599 and 4.721/3.412; oneMKL had 5.698/3.532 and 4.426/2.690. Mean compute favored oneMKL by about 11.3%, but mean end-to-end analysis was 5.062 seconds oneMKL versus 4.922 OpenBLAS. All 85,437 retained rows matched in IDs/order/N/DF; maximum R-squared/T/log10-p differences were `1.94e-16`, `2.31e-14`, and `1.99e-13`. The largest absolute effect difference, 0.03125, was `6.67e-16` relative on an approximately `-4.69e13` effect. This mixed evidence does not justify changing the release default. The requested 15 BLAS threads were honored by OpenBLAS; the oneMKL runtime reported eight.
- Production BCF validation used the batch1234 `freeze.12c.chr22-fhs-framid.vcf.gz` through a temporary official MSYS2 bcftools 1.24/htslib 1.24 extraction under ignored `target`; the downloaded MSYS2 archive SHA-256 was `c105946e64e08f099ac0e4647461ce762b95333ad211777666476a9a41451d65`. bcftools extracted 4,059 records from `chr22:20000000-20009999` and wrote both tabix VCF and standard-CSI BCF; an HTSJDK test utility created the supported Tribble `.idx` for the BCF. Overlapping regions `chr22:20001000-20005999` and `chr22:20004000-20008999` produced a 3,321-record union. Application VCF/TBI and BCF/Tribble runs both used indexed access, aligned 4,746 of 7,214 source samples, and reported input 3,321, included 290, monomorphic 3,031, singleton 92, doubleton 41. Their QC files were byte-identical (SHA-256 `A0C62C073A43954D6FFE627FE11D4CB9D821CD459872DDC57CA2372B69753FF8`). Canonical IDs/order and A-only/A+B/B-only memberships (1,190/815/1,316) matched bcftools exactly. Sample-subset bcftools fill-tags matched integer AC/AN/NS exactly; maximum displayed MAF and HWE deltas were `4.42e-7` and `4.99e-7`, consistent with bcftools query text precision.
- BCF interruption command added `-Deqtl.variant.qc.checkpoint.records=512 -Deqtl.test.variant.qc.fail.after=1024`; it left two durable parts. The rerun sought directly to boundary `chr22:20003545:ACT:A` through the Tribble index and reassembled the same QC hash. A standard-CSI region run against the final jar failed clearly before analysis and explicitly required `.tbi` or `.idx`; no sequential fallback occurred.
- `java -jar target\gpu-eqtl-2.0.0-SNAPSHOT-all.jar --printbackendinfo` detected NVIDIA GeForce RTX 2080, CUDA driver API 13.3, CUDA runtime 12.6, compute capability 7.5, compiler/NVRTC availability, FP64, and 8,589,606,912 bytes VRAM. CPU detection reported bundled OpenBLAS 0.3.34 with 15 BLAS threads; no usable OpenCL device was present. `git diff --check` reported no whitespace errors, only informational LF-to-CRLF working-copy warnings.

### Compatibility, limitations, and next step

- Positive explicit set-block sizes retain their prior meaning. Automatic sizing is conservative but is an estimator, not a hard JVM allocation limit; pathological allocator pressure may still warrant a smaller explicit override. Tile-major result order can change when an automatically resolved size differs, while identifiers/statistics and the set audit remain deterministic for that resolved schedule.
- Genotype-outer pattern frequency candidate counts are now durable scientific audit state, but they do not change variant inclusion. Keep `--frequency-scope pattern` on the established pattern-outer scheduler until block-local filtering, comparison progress, and result-order consequences are implemented and tested.
- The production source's neighboring `.tbi` was older than the VCF according to bcftools, so validation operated on a copied slice with newly generated indexes. Standard CSI remains worth supporting, but doing so requires a deterministic pure-Java CSI decoder or a carefully licensed/packaged cross-platform htslib bridge; current tabix VCF and Tribble BCF are validated and unsupported CSI never degrades to a scan.
- The ordinary release remains the universal OpenBLAS jar; do not switch to oneMKL based on a small compute-only advantage without stable full-run evidence across representative platforms. Next recommended work is either cross-platform release smoke/checksum publication or enabling pattern-specific MAF/MAC in genotype-outer using the now-durable audit counts and scalar pattern-outer oracle.

## 2026-08-28T02:12:17-04:00 — Pattern filtering, cohort models, cache lifecycle, and release verification

### Baseline and goal

- Baseline commit: `21701518790a9a63a5f2e69f1a8414e892128a56` (`Record completed optimization and validation milestones`) on `master`; both configured remotes were clean and matched at the start.
- Goal: finish the next ordered milestones—enable genotype-outer pattern-specific frequency filtering, establish cohort-aware ordinary eQTL analysis, add safe cache operations, and make release packaging reproducible—then commit and push both remotes.

### Decisions and files changed

- Enabled `--frequency-scope pattern` in genotype-outer exact deletion. Per-model inclusion masks now apply MAF/MAC after each exact trait mask, association output omits excluded pairs, QC `included_variants` reflects the applied decision, dynamic progress discovers and counts only active comparisons, resume reconstructs its active total from checksummed QC parts, and version-2 result/QC signatures include scope and thresholds. The established pattern-outer oracle and genotype-outer scheduler retain the same statistics, identifiers, and deterministic order for retained pairs.
- Added `--cohort-model TSV` plus `--cohort-column`. `QCohortModel` builds exact cohort-specific intercept/fixed/factor blocks, validates full rank and residual DF, supports identity, aligned covariance-matrix, compound-symmetry repeated-observation, and pre-aggregated `one-per-subject` contracts, applies one Cholesky whitening transformation to both predictor and trait rows, binds the transformation to prepared-cache signatures, and writes `<output>.cohorts.tsv`. Cohort covariance is deliberately rejected for set tests and exact post-whitening trait masks until those null/missingness contracts are defined.
- Added backend-free `--inspect-cache`, `--cache-report`, `--prune-cache`, `--prune-cache-older-than-days`, and `--apply-cache-prune`. Inventory reports type/size/time/SHA-256 readability; pruning recognizes only declared cache artifacts, is dry-run by default, probes actual file locks rather than treating stale lock files as active, skips unreadable/empty/locked candidates, and moves applied files into timestamped cache-local `.trash` paths instead of deleting them.
- Added the `cpu-openblas-platform` Maven profile plus Windows/POSIX release-verification scripts. They require a clean publish tree (with an explicitly recorded dirty development override), use the source commit timestamp, run tests unless skipped, validate packaged GPL/exception/vendor notices, smoke-test the jar, and emit `SHA256SUMS` plus a release manifest. Shade configuration merges services/the executable manifest and omits conflicting versioned module descriptors while preserving dependency Maven metadata required by JavaCPP. Added a supported-runtime checklist and snapshot release notes; updated README, command reference, TODO, and `.gitignore` for platform build directories.
- Main implementation files include `QAnalysisProgress`, `QGenotypeOuterPatternJob`, `QGenotypeOuterPatternQcCheckpoint`, `QeQTLAnalysis`, `QeQTLAnalysisConfig`, `QeQTLCommandLine`, `QCovariateTable`, new `QCohortModel`, and new `QCacheManager`. Added/expanded deterministic progress, trait-pattern, cohort, cache, and CLI tests.

### Verification

- `.\mvnw.cmd -q '-Dtest=QTraitPatternAnalysisTest,QAnalysisProgressTest' test` — passed after correcting fixed-total progress completion reporting. The pattern-MAC fixture retained seven active genotype-outer comparisons versus eight aligned-scope comparisons and matched pattern-outer retained statistics/QC.
- `.\mvnw.cmd -q '-Dtest=QCohortModelTest' test` — passed. The fixture covers cohort-specific designs, repeated subjects, common whitening/cache tags, stacked-versus-blockwise products, and byte-identical resident versus prepared-cache-streamed association output.
- `.\mvnw.cmd -q '-Dtest=QCacheManagerTest,QCohortModelTest,QeQTLCommandLineTest' test` and the final `.\mvnw.cmd -q '-Dtest=QCacheManagerTest,QCohortModelTest,QTraitPatternAnalysisTest,QAnalysisProgressTest,QeQTLCommandLineTest' test` — passed. Cache tests hold a real file lock, prove dry-run reporting, move only the unlocked recognized artifact, preserve unrelated data, and find the recoverable trash copy.
- `.\scripts\verify-release.ps1 -Platform windows-x86_64` — clean package workflow passed the complete suite: 130 tests, zero failures, zero errors, and five intentional runtime/platform skips. It built the classifier-specific jar, found `META-INF/LICENSE`, `META-INF/LICENSE_EXCEPTION`, and all three `META-INF/licenses` notices, and passed packaged backend discovery.
- After the live-lock/provenance/metadata refinements, `.\scripts\verify-release.ps1 -Platform windows-x86_64 -SkipTests -AllowDirty` — development package and smoke passed. The deliberately dirty manifest prevents publication; jar SHA-256 was `2ea2ae99fc6c6fac96709b1761a898720b600021e7725bb781c2b8da31dd2027`. JavaCPP reported OpenBLAS normally after retaining Maven version metadata. Remaining shade messages are reviewed duplicate upstream manifest/license/notice resources, not missing project/vendor notices.
- Packaged backend smoke detected NVIDIA GeForce RTX 2080, CUDA driver API 13.3, CUDA Runtime 12.6, compute capability 7.5, compiler available, FP64, and 8,589,606,912 bytes VRAM. CPU detection loaded bundled OpenBLAS 0.3.34 with 15 BLAS threads on Windows 11; no usable OpenCL device was present. No new production GPU association was required because the genotype-outer filter operates on already validated compact CUDA/CPU statistics and its retained results were anchored to the CPU pattern oracle.
- PowerShell parsed `scripts/verify-release.ps1`; no usable POSIX distribution was installed on this Windows host, so `scripts/verify-release.sh` remains a target-host gate. `git diff --check` reported no whitespace errors, only informational LF-to-CRLF warnings.

### Compatibility, limitations, and next step

- FP64 remains the default; existing aligned-scope behavior, version-1 matrix/raw caches, output schemas, and pattern-outer scheduling remain compatible. Genotype-outer version-2 association/QC signatures intentionally do not reuse version-1 blocks because inclusion and progress semantics changed.
