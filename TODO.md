...
- Replace the pattern-outer prepared-predictor schedule with a scalable genotype-block-outer or sufficient-statistics implementation. It must preserve each trait's exact sample set, projection, rank, threshold, output order, N, DF, pattern-specific genotype imputation/statistics, and restart determinism without writing one full prepared genotype cache per pattern. The batch1234 incomplete expression file has 178,105 traits, 275,746,428 missing values, no complete trait rows, and 16,862 exact masks (observed N 20–4,745). Production preflight estimated 7,771.9 full-cohort predictor-preparation passes and 118.19 TiB of prepared numeric values; 727 patterns covering 2,967 traits cannot support the 31-column covariate model plus a tested predictor. The current correct/restartable schedule is therefore not operationally suitable for that entropy.
- Persist/sign the exact matrix missingness scan so repeat launches do not reread the full 3.95-GB expression CSV merely to reconstruct unchanged row masks. Keep audit regeneration deterministic and invalidate reuse on source metadata, selected sample order, delimiter/parser behavior, or missing-token policy changes.
