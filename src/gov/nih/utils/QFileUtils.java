...
public class QFileUtils
{
	...
	public static final QTableDataExtra readDelimitedFileAsTableDataExtra(String filename, char[] delim, String comments,
		int numRowNames, int numColNames, boolean isAllNumeric) throws IOException
	{	return readDelimitedFileAsTableDataExtra(new BufferedReader(new FileReader(filename), kDefaultBufferSize), delim, comments, numRowNames, numColNames, isAllNumeric); }

	public static final QTableDataExtra readDelimitedFileAsTableDataExtra(File f, char[] delim, String comments,
		int numRowNames, int numColNames, boolean isAllNumeric) throws IOException
	{	return readDelimitedFileAsTableDataExtra(new BufferedReader(new FileReader(f), kDefaultBufferSize), delim, comments, numRowNames, numColNames, isAllNumeric); }

	public static final QTableDataExtra readDelimitedFileAsTableDataExtra(Reader r, char[] delim, String comments,
		int numRowNames, int numColNames, boolean isAllNumeric) throws IOException
	{
		if (numRowNames < 0 || numColNames < 0)
			throw new RuntimeException();
		CsvReader reader = null;
		List<double[]> list = new ArrayList<double[]>();
		String[][] colNames = numColNames > 0 ? new String[numColNames][] : null;
		List<String>[] rowNameList = null;
		Set<String>[] colcats = null;
		if (numRowNames > 0)
		{
			rowNameList = new List[numRowNames];
			for (int i = 0; i < numRowNames; i++)
				rowNameList[i] = new ArrayList<String>();
		}
		...
		try
		{
			...
			if (numColNames > 0) {
				for (int i = 0; i < numColNames; i++) {
					lineNo++;
					boolean b = reader.readRecord();
					if (!b)
						return null;
					colNames[i] = reader.getValues();
					if (numRowNames > 0) {
						String[] s = new String[colNames[i].length - numRowNames];
						System.arraycopy(colNames[i], numRowNames, s, 0, s.length);
						colNames[i] = s;
					}
				}
			}
			while(reader.readRecord()) {
				...
				if (numTokens == 0) {
					...
					if (!isAllNumeric)
						colcats = new Set[colNames.length];
				} else
				...
				if (numRowNames > 0)
				{
					curData = new double[numTokens - numRowNames];
					for (int i = 0; i < numRowNames; i++)
					{
						tok = tokens[i];
						rowNameList[i].add(tok);
					}
					for (int i = numRowNames; i < curNumTokens; i++)
					{
						tok = tokens[i];
						try {
							curData[i - numRowNames] = Double.parseDouble(tokens[i]);
						} catch (NumberFormatException e) {
							int im1 = i - numRowNames;
							if (isAllNumeric)
								curData[im1] = kUndefinedValue;
							else {
								if (colcats[im1] == null) {
									colcats[im1] = new HashSet<String>();
									for(double[] vals: list)
										colcats[im1].add(String.valueOf(vals[im1]));
								}
								colcats[im1].add(tok);
							}
						}
				...
				else
				{
					...
					for (int i = 0; i < curNumTokens; i++)
					{
						tok = tokens[i];
						try {
							curData[i] = Double.parseDouble(tokens[i]);
						} catch (NumberFormatException e) {
							if (isAllNumeric)
								curData[i] = kUndefinedValue;
							else {
								if (colcats[i] == null) {
									colcats[i] = new HashSet<String>();
									for(double[] vals: list)
										colcats[i].add(String.valueOf(vals[i]));
								}
								colcats[i].add(tok);
							}
						}
		...
		if (size == 0)
			return null;
		String[][] rowNames = numRowNames > 0 ? new String[numRowNames][] : null;
		for (int i = 0; i < numRowNames; i++)
		{
			rowNames[i] = rowNameList[i].toArray(new String[size]);
			rowNameList[i].clear();
		}
		String[][] colcatsArray = null;
		if (!isAllNumeric) {
			...
			for (int i = 0; i < colNames.length; i++)
				if (colcats[i] != null) {
					colcatsArray[i] = colcats[i].toArray(new String[colcats[i].size()]);
					List<String> cats = new ArrayList<String>();
					cats.addAll(colcats[i]);
					for(double[] vals: list)
						vals[i] = cats.indexOf(String.valueOf(vals[i]));
				}
		}
		QTableDataExtra tbl = new QTableDataExtra(list.toArray(new double[size][]), rowNames, colNames);
		if (colcatsArray != null)
			tbl.setColumnCategories(colcatsArray);
		return tbl;
	}
